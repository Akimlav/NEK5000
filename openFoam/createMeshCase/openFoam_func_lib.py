...
def createCase (i, case_path, mesh_path):
    ...
    src = mesh_path + '/constant/polyMesh'
    ...
    path4 = case_location + '/'+case_path
    path5 = case_location + '/'+case_path + '/run.sh'
    replaceAll(path5, "loc1", case_path)
    with fileinput.FileInput(path4 + '/run.sh' , inplace=True) as file:
        for line in file:
            print(line.replace('loc', path4 ), end='')
    return()
...
def createMesh (i, mesh_path):
    ...
    replaceAll(path1, 'cells*', str(int(mesh[i][0])))
