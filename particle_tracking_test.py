...
for data in range(0,5):
    ...
    # deposited.append(str(len(data)))
    deposited['sum'] = len(data)
    deposited['hot_wall'] = len(hot_wall)
    deposited['cold_wall'] = len(cold_wall)
    deposited['cold_ceiling'] = len(cold_ceiling)
    deposited['hot_floor'] = len(hot_floor)
    deposited['adiabatic_front'] = len(adiabatic_front)
    deposited['adiabatic_back'] = len(adiabatic_back)
    
    depositedDict[str(np.round((data[0,2] * 1.22 * 1e6), 2))] = deposited
...
# plt.hist(depositedDict['0.1'])
#
# cold_c = depositedDict['0.5']['cold_ceiling'] / depositedDict['0.5']['sum']
# cold_w = depositedDict['0.5']['cold_wall'] / depositedDict['0.5']['sum']
