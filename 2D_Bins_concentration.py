...
for t in range(len(allFileList)):
    ...
    for i in range(len(A[0])):
        for j in range(len(A[0])):
                CC[i,j] = (A[t][i,j]/delta**3 - Cinf)**2
    sigma = np.sqrt((sum(sum(CC)))/n**3)
    # print(sigma)
