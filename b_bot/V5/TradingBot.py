...
def BacktestStrategies(symbols=[], interval='1w', plot=False, strategy_evaluators=[], 
options = dict(starting_balance=1, initial_profits = 1.012, initial_stop_loss = 0.9, 
...
def Main():
	...
	exchange = Binance()
	symbols = exchange.GetTradingSymbols(quoteAssets=["BNB"])
	...
	if answer == 'e':
		EvaluateStrategies(symbols=symbols, interval='4h', strategy_evaluators=strategy_evaluators)
	if answer == 'b':
		BacktestStrategies(symbols=symbols, interval='4h', plot=False, strategy_evaluators=strategy_evaluators)
	if answer == 'q':
		print("\nBYE!\n")
