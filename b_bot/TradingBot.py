...
def BacktestStrategies(symbols=[], interval='5h', plot=False, strategy_evaluators=[],
options = dict(starting_balance=1, initial_profits = 1.01, initial_stop_loss = 0.9,
		...
		for evaluator in strategy_evaluators:
			if evaluator.evaluate(model):
				...
				if answer == 'p':
					...
					order_result = model.exchange.PlaceOrder(model.symbol, "BUY", "MARKET", quantity=0.11, test=False)
